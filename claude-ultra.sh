...
# Créer le TODO.md spécifique pour un agent (une seule tâche)
create_agent_todo() {
    local worktree_path="$1"
    local task="$2"
    
    cat > "${worktree_path}/${TASK_FILE}" << EOF
# TODO - Agent Task
...
draw_swarm_dashboard() {
    ...
    for ((i=0; i<PARALLEL_AGENTS; i++)); do
        ...
        # Récupérer la tâche
        local task=""
        if [ -f "${worktree_path}/${TASK_FILE}" ]; then
            task=$(grep -E "^\s*- \[ \]" "${worktree_path}/${TASK_FILE}" 2>/dev/null | head -1 | sed 's/^[[:space:]]*- \[ \][[:space:]]*//' | cut -c1-40)
        fi
...
while true; do
    ...
    for ((i=0; i<PARALLEL_AGENTS; i++)); do
        ...
        # Récupérer la tâche
        task=""
        if [ -f "${worktree_path}/TODO.md" ]; then
            task=$(grep -E "^\s*- \[ \]" "${worktree_path}/TODO.md" 2>/dev/null | head -1 | sed 's/^[[:space:]]*- \[ \][[:space:]]*//' | cut -c1-50)
        fi
